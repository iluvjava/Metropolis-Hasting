...
# function run_experiment()
    ...
    n = 10                                              # items not part of the solutions!  
    ...
    bcrw = ISRW(zeros(Int, n + m), ones(Int, n + m), 3)    # The base chain. 
    ...
    function objval(x)
        ...
        return exp(dot(weights, x)/0.05)
    end
    global mhc = MHC((x) -> objval(x), bcrw, zeros(Int, m + n))
    global SAMPLED = [mhc() for _ in 1: 100000]
